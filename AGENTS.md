# Agent Guide — elm-lisp-interpreter
...
- **Source directories:** `src/` is declared in `source-directories`;
  ...
  adding directories
- **Review scaffold:** `review/src/ReviewConfig.elm` holds a minimal rule set
  (debug-log bans, unused variables); extend it deliberately as the code grows
...
- **elm-format** is the Elm formatter (`pnpm run format` applies it;
  `--validate` in lint). Generated code is always in `elm-format` style
- **elm-review** is the linter with the project rule set in `review/` — do not
...
- `pnpm lint` — elm-format validation, elm-review, Biome
- `pnpm test` — `elm-test` suite
...
- **No comments:** Do not add comments to source files. The code should be
  self-documenting
- **Functional style:** Pure functions, no side effects outside `main`
...
A lisp interpreter written in Elm, published as a learning exploration of parsers (PEG grammars via elm-peg) and interpreters.
...
- **CI** (`continuous-integration.yml`): Runs on PRs to `main` and
  `workflow_dispatch`. Jobs: `lint` and `test` via composite actions. Concurrency
...
- **Parser robustness:** broaden PEG grammar coverage and error reporting
